...
TMP_ACPICA="${TOP_DIR}/acpica_port_commits_${SUFFIX}"
...
cleanup() {
  ...
  # Remove any patch reject/backup artifacts left in the kernel tree.
  if [[ -n "${KERNEL_DIR}" && -d "${KERNEL_DIR}" ]]; then
    find "${KERNEL_DIR}" -type f \( -name "*.rej" -o -name "*.orig" \) -delete >/dev/null 2>&1 || true
  fi
}
trap cleanup EXIT
...
for CMT in "${COMMITS[@]}"; do
  ...
  cd "${TMP_ACPICA}"
  echo "Processing commit ${CMT} from ACPICA:"
  ...
  for FILE in "${CHANGED_FILES[@]}"; do
    ...
    if [ -n "$DIFF_RESULT" ]; then
        ...
        if patch -l -F 3 -f --dry-run -d "${DEST_DIR}" "$FILE_NAME" <<< "$DIFF_RESULT" >/dev/null 2>&1; then
            patch -l -F 3 -d "${DEST_DIR}" "$FILE_NAME" <<< "$DIFF_RESULT"
        else
            echo "Conflict: patch does not apply cleanly to ${DEST}:"
            # patch -l -F 3 -f --dry-run -d "${DEST_DIR}" "$FILE_NAME" <<< "$DIFF_RESULT" || true
            # Offer the same choices as 'git rebase' on a conflict.
            while true; do
                read -p "Choose: [c]ontinue / [s]kip / [a]bort: " ans < /dev/tty
                case "${ans}" in
                    c|continue )
                        patch -l -F 3 -d "${DEST_DIR}" "$FILE_NAME" <<< "$DIFF_RESULT" || true
                        ANYTHING_REJECTED=1
                        break ;;
                    s|skip )
                        SKIP_COMMIT=1
  ...
  if [[ "${SKIP_COMMIT}" == 0 && "${ANYTHING_REJECTED}" != 0 ]]; then
    cd "${KERNEL_DIR}"
    echo "Check the .rej files if any, and apply changes manually before continuing"
    while true; do
      read -p "Choose: [c]ontinue / [s]kip / [a]bort: " ans < /dev/tty
      case "${ans}" in
        c|continue )
